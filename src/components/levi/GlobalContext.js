...
export const GlobalProvider = ({children}) => {
  ...
  const backgroundSyncData = async () => {
    ...
    try {
      ...
      // Silent sync of harvest checks
      await services.synchronizeHarvestChecks(
        instance,
        false, // Don't force - only sync unsynced records
        null, // No loading state updates
        null, // No heading updates
        null, // No loading text updates
        fetchOfflineEventsCount, // Update offline count
      );

      // Silent sync of QC field assessments
  ...
  const backgroundSyncLookups = async () => {
    ...
    try {
      ...
      // Silent sync of lookups with dummy UI functions
      await services.syncLookups(
        instance,
        currentClientId,
        JSON.stringify(departments || []),
        isConnected,
        silentLoading, // Dummy function instead of null
        silentHeading, // Dummy function instead of null
        silentText, // Dummy function instead of null
      );
  ...
  const synchData = async (showLoader = true, setLoading = null, setLoaderHeading = null, setLoadingText = null) => {
    ...
    try {
      ...
      // Sync harvest checks with UI updates if provided
      await services.synchronizeHarvestChecks(
        instance,
        true, // Force sync all records
        showLoader ? setLoading : null,
        showLoader ? setLoaderHeading : null,
        showLoader ? setLoadingText : null,
        fetchOfflineEventsCount,
      );

      // Sync QC field assessments with UI updates if provided
      await services.synchronizeQcFieldAssessments(
        instance,
        true,
        showLoader ? setLoading : null,
  ...
  const handleNetworkChange = state => {
    ...
    const connectionMap = {
      ...
      cellular: `Cellular (${
        state.details.cellularGeneration || 'generic'
      }) connection`,
